 ...
 */
public class PolyhedronDfsDraft2 implements TranverserInterface {
    ...
    public PolyhedronDfsDraft2(SubgraphMapperInterface mapper) {
        this.mapper = mapper;
    }
    ...
    protected boolean dfs(LinkedHashSet<SubgraphInterface> faces,
        ...
        for (int nodeId : GraphUtils.getAdjacentVertices(currentNode.getId(), adjacencyMatrix)) {
            ...
            if (nodeId == requiredNode.getId()) {
                currentPath.addEdge(edge);
                return checkAngle(faces, currentPath);
            }
     ...
     */
    private boolean checkAngle(LinkedHashSet<SubgraphInterface> faces, SubgraphInterface currentPath) {
